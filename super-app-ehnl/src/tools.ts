...
export function setupServerTools(server: McpServer) {
  ...
  server.tool(
  "sentinel-analyze-transaction",
  "Analyze a transaction using Sentinel governance agents",
  {
    user: z.string().describe("User making the transaction"),
    amount: z.number().describe("Transaction amount")
  },
  async ({ user, amount }) => {

    const response = await fetch("http://localhost:5000/intercept/transfer", {
      method: "POST",
      headers: {
...
server.tool(
  "sentinel-get-governance-logs",
  "Fetch recent governance decisions from Sentinel",
  {
  ...
  async ({ limit = 10 }) => {
    ...
    const response = await fetch(
      `http://localhost:5000/governance/logs?limit=${limit}`
    );
...
server.tool(
  "sentinel-suggest-policy",
  "Suggest a new governance policy based on suspicious patterns",
  {
    description: z.string().describe("Observed suspicious behavior")
  },
  async ({ description }) => {

    const response = await fetch(
      "http://localhost:5000/ai/suggest-policy",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
